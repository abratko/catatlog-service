...
import (
	...
	"gitlab.trgdev.com/gotrg/white-label/services/catalog/internal/category/tests"
)
...
func TestCategoryRepo_FindByIds(t *testing.T) {
	...
	mockFetch := func(ctx context.Context) (chan dto.Category, error) {
		ch := make(chan dto.Category, len(categories))
		go func() {
			defer close(ch)
	...
	repo := app.NewCategoryRepo(mockFetch, "3m")
	...
	tests := []struct {
		...
		{
			name: "find single category",
			ids:  []string{"1"},
			want: 1,
		},
		{
			name: "find multiple categories",
			ids:  []string{"1", "2"},
			want: 2,
		},
		{
			name: "find non-existent category",
			ids:  []string{"999"},
			want: 0,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(got) != tt.want {
				t.Errorf("FindByIds() got %v categories, want %v", len(got), tt.want)
			}
		})
	}
}
...
func TestCategoryRepo_Create(t *testing.T) {
	...
	t.Run("init during creation and reinit after expiration", func(t *testing.T) {
		...
		expEsReqPath := "/prefix_category/_search"
		...
		result1, err := repo.FindByIds(context.Background(), []string{"25", "26"})
		...
		result2, err := repo.FindByIds(context.Background(), []string{"33"})
		repo.CancelReinit()
